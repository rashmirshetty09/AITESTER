...
Parse the Jira ticket key from the chat message.
Fetch ticket details (summary, description, acceptance criteria) via the Jira REST API using the stored credentials.
Load the test case template from a local /templates folder.
Generate test cases with the selected LLM, using the fetched ticket content merged into the template structure.
Render the generated test cases back in the chat pane.
